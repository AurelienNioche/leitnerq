...
#              [np.mean(x) for x in recall_rates],
#              yerr=[std_err(x) for x in recall_rates])
...
# x = x[(~np.isnan(x)) & (x > 0)]
# plt.hist(np.log10(1 + x), alpha=0.5, label='forgotten', normed=True,
#          linewidth=0)  # , bins=20)
#
# x = np.array(df[df['outcome'] == 1]['delay'].values)
# x = x[(~np.isnan(x)) & (x > 0)]
# plt.hist(np.log10(1 + x), alpha=0.5, label='recalled', normed=True,
#          linewidth=0)  # , bins=20)
#
# plt.legend(loc='best')
