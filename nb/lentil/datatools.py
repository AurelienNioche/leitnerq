...
class InteractionHistory(object):
    ...
    def __init__(
            self,
            data,
            sort_by_timestep=False,
            reindex_timesteps=False,
            size_of_test_set=0.2):
        """
        Initialize interaction history object

        :param pd.DataFrame data: An interaction history dataframe
        ...
        :param bool reindex_timesteps:
            True => reindex timesteps to 1,2,3,4,...
            (from something like 1,3,7,8,...)

        :param float size_of_test_set:
        Fraction of students to include in test set, where
            0 <= size_of_test_set < 1
            (size_of_test_set = 0 => don't use a test set)
        """
    ...
    def split_interactions_by_type(
        ...
        :param pd.DataFrame|None filtered_history:
        A filtered interaction history
        :param bool insert_dummy_lesson_ixns:
            If there are no lesson interactions in the history,
            and assessment interactions occur
            at different timesteps for at least one student,
            and you want to assume that student
            embeddings follow a temporal process, then set to True.

            If there are lesson interactions in the history,
        ...
        def df_to_unique_timesteps(dataframe):
            """ Convert a dataframe's timesteps so
            that only one event happens per timestep """
            output = \
                dataframe['student_id'].apply(self.idx_of_student_id).values
            return output * num_timesteps + dataframe['timestep'].values

        assessment_df = \
            df[df['module_type'] == AssessmentInteraction.MODULETYPE]
        assessment_timesteps = df_to_unique_timesteps(assessment_df)
        assessment_idxes = \
            assessment_df['module_id'].apply(self.idx_of_assessment_id)
        ...
        assessment_interactions = \
            (np.array(assessment_timesteps, dtype=np.int),
    ...
    def iter_assessments(self):
        ...
        return self._assessment_idx.keys()
