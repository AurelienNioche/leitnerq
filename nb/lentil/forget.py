...
class LinearLUVModel(TimeVaryingLUVModel):
    ...
    def learning_update_variances(
        ...
        return \
            self.alpha * np.log(times_since_prev_ixn_for_lesson_ixns+1) \
            + self.beta
...
class LinearForgettingModel(ForgettingModel):
    ...
    def __init__(self, alpha, beta):
        ...
        :param float alpha: Coefficient, which controls how sensitive
        the forgetting penalty is to log(time since previous interaction)
...
class LogisticForgettingModel(ForgettingModel):
    ...
    def __init__(self, alpha, beta):
        ...
        which controls the baseline forgetting penalty
        """
